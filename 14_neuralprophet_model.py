...
print("="*80)
print("NEURAL PROPHET DEEP LEARNING FORECASTING MODEL")
print("Built by Facebook/Meta - Designed for Business Time Series")
...
# ============================================================================
# STEP 3: Train NeuralProphet Per Region
# ============================================================================
print("\nSTEP 3: Training NeuralProphet per Region...")
...
for region in regional_df['Region'].unique():
    rdf = regional_df[regional_df['Region']==region].copy()
    rdf = rdf.sort_values('Date')

    if len(rdf) < 24:
        print(f"   {region:<20} : Skipped (only {len(rdf)} months)")
        continue

    np_df = rdf.rename(columns={
        'Date'          : 'ds',
        'Sales_Quantity': 'y'
    })

    train_np = np_df.iloc[:-6].copy()
    test_np  = np_df.iloc[-6:].copy()

    try:
        # Fixed NeuralProphet initialization
        m = NeuralProphet(
        ...
        m.add_lagged_regressor('Eid_Indicator')
        m.add_lagged_regressor('Avg_Temperature')
        ...
        future['Eid_Indicator']   = future['ds'].apply(
            lambda x: 1 if x.month in [3,4] else 0)
        future['Avg_Temperature'] = future['ds'].apply(
            lambda x: [12,15,21,27,32,35,
                        34,32,30,24,18,13][x.month-1])

        forecast = m.predict(future)

        # Test predictions
        test_preds = forecast.tail(6)['yhat1'].values
        test_preds = np.maximum(0, test_preds)
        actuals    = test_np['y'].values
        ...
        jan_forecast = max(0, forecast['yhat1'].iloc[-1])
...
print(f"\n{'='*60}")
print(f"NEURAL PROPHET ACCURACY METRICS")
print(f"{'='*60}")
...
fig.suptitle('NeuralProphet Forecasting Results\n'
             'Facebook/Meta Time Series Model',
...
if len(results_df) > 0:
    ...
    axes[0,0].set_title('NeuralProphet MAPE by Region',
                        fontweight='bold')
...
axes[1,1].axis('off')
summary_txt = f"""
NEURAL PROPHET SUMMARY

Built by     : Facebook / Meta
Type         : Deep Learning
Handles      : Seasonality + Holidays + Trend
...
plt.tight_layout()
plot_file = r'C:\Users\User\Documents\FPY\neuralprophet_visualization.png'
...
output = r'C:\Users\User\Documents\FPY\neuralprophet_results.xlsx'
...
# ============================================================================
print("\n" + "="*80)
print("NEURAL PROPHET MODEL COMPLETE")
print("="*80)
print(f"\nNeuralProphet Accuracy : {med_mape:.2f}% MAPE")
print(f"\nAll 9 Models:")
...
print(f"  9. NeuralProphet   : {med_mape:>7.2f}%  Deep Learning")
print(f"\nNational Base   : {national_base:,.0f} units")
print(f"National +25%   : {national_growth:,.0f} units")
print(f"\nFiles saved:")
print(f"  1. neuralprophet_results.xlsx")
print(f"  2. neuralprophet_visualization.png")
print("="*80)
